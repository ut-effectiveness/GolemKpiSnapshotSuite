...
#' @param custom_server Optional override (unused here).
#' @export
mod_ipeds_server <- function(id,
  ...
  moduleServer(id, function(input, output, session) {
    ...
    # Normalize device each time (helper defined in mod_utils.R)
    observe({
      dev <- normalize_device(device_type)
      if (dev == "desktop") {
        # desktop-specific code (if any)
      } else {
    ...
    output$line_plot_2 <- plotly::renderPlotly({
      ...
      gg <- ggplot2::ggplot(
        ...
        ggplot2::aes(
          ...
          date = .data[[date_var]],
          text = paste("Headcount:", .data[[text_var]])
        )
      ) +
        ...
        ggplot2::labs(x = "Days to class start", y = "Headcount") +
